...
def meals(date, id):
    ...
    c = cursor.execute("SELECT calories from date WHERE people_id = (?) and date = (?)", (id, date)).fetchone()
    finalc = float(c[0])
...
def log(id):
    ...
    while True:                            
        try:
            # result is the remaining calories that still need to be consumed
    ...
    if result >= 0:
        print(f'You did not meet your calories goal that day, you need to consume{result} more calories')
